...
namespace acid
{
	...
	{
	private:
		std::unique_ptr<RenderManager> m_managerRender;
	...
	public:
		...
		RenderManager *GetManager() const { return m_managerRender.get(); }

		/// <summary>
		/// Sets the current renderer manager to a new renderer manager.
		/// </summary>
		/// <param name="rendererMaster"> The new renderer manager. </param>
		void SetManager(RenderManager *managerRender) { m_managerRender.reset(managerRender); }
